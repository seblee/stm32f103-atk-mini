...
void CC1101_Write_Cmd(uint8_t Command)
{
    ...
    rf_spi_read_write_byte(Command); //写命令
...
void CC1101_Write_Reg(uint8_t Addr, uint8_t WriteValue)
{
    ...
    rf_spi_read_write_byte(Addr);       //写地址
    rf_spi_read_write_byte(WriteValue); //写数据
...
void CC1101_Write_Multi_Reg(uint8_t Addr, uint8_t *pWriteBuff, uint8_t WriteSize)
{
    ...
    rf_spi_read_write_byte(Addr | WRITE_BURST); //连续写命令 及首地址
    for (i = 0; i < WriteSize; i++)
    {
        rf_spi_read_write_byte(*(pWriteBuff + i)); //连续写入数据
    }
...
uint8_t CC1101_Read_Reg(uint8_t Addr)
{
    ...
    rf_spi_read_write_byte(Addr | READ_SINGLE); //单独读命令 及地址
...
uint8_t CC1101_Read_Status(uint8_t Addr)
{
    ...
    rf_spi_read_write_byte(Addr | READ_BURST);  //连续读命令 及地址
    l_RegStatus = rf_spi_read_write_byte(0xFF); //读取状态
...
void CC1101_Read_Multi_Reg(uint8_t Addr, uint8_t *pReadBuff, uint8_t ReadSize)
{
    ...
    rf_spi_read_write_byte(Addr | READ_BURST); //连续读命令 及首地址
    for (i = 0; i < ReadSize; i++)
    {
        for (j = 0; j < 20; j++)
            ;
        *(pReadBuff + i) = rf_spi_read_write_byte(0xFF); //连续读取数据
    }
...
{
    uint8_t i = 0;
    uint8_t PARTNUM_OF_CC = 0;
    uint8_t VERSION_OF_CC = 0;

    CC1101_Gpio_Init(); //引脚初始化
    CC1101_Reset();     //模块复位

    PARTNUM_OF_CC = CC1101_Read_Reg(CC1101_PARTNUM);
